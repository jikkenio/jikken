...
mod tests {
        ...
        fn on_event(&mut self, event: ExecutionEvent) {
            if let Ok(mut events) = self.events.lock() {
                match event {
                    ...
                    ExecutionEvent::TestStart { test, iteration, .. } => {
                        events.push(format!("TestStart: {} iteration {}", 
                            test.name.clone().unwrap_or_default(), iteration));
                    }
                    ExecutionEvent::TestComplete { test, .. } => {
                        events.push(format!("TestComplete: {}", 
                            test.name.clone().unwrap_or_default()));
                    }
                    ExecutionEvent::TestSkipped { test, reason } => {
                        events.push(format!("TestSkipped: {} ({})", 
                            test.name.clone().unwrap_or_default(), reason));
                    }
                    ExecutionEvent::SuiteComplete { total_tests, passed, failed, .. } => {
                        events.push(format!("SuiteComplete: {}/{} passed, {} failed", 
    ...
    async fn test_observer_integration_with_empty_tests() {
        ...
        assert!(!captured_events.is_empty(), "Should have emitted some events");
    ...
    fn test_execution_events_are_cloneable() {
        ...
        match (event, cloned_event) {
            (ExecutionEvent::SuiteStart { total_tests: t1, .. }, 
             ExecutionEvent::SuiteStart { total_tests: t2, .. }) => {
                assert_eq!(t1, t2);
            }
